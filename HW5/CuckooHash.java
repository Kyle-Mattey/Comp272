...
public class CuckooHash<K, V> {
    ...
    public void put(K key, V value) {
        ...
        // Step 1: Check if the key-value pair is already in the table
        if ((table[position1] != null && table[position1].getBucKey().equals(key) && table[position1].getValue().equals(value)) ||
            ...
            return; // Key-value pair is already in the table; no action needed
        }

        // Step 2: Insert key-value pair, handling collisions
        int attempts = 0;
        while (attempts < CAPACITY) {
            if (table[position1] == null) {
                table[position1] = newBucket; // Insert into position1 if empty
                return;
            }

            // If position1 is occupied, kick out the existing bucket and try to place it at its alternate hash location
            Bucket<K, V> kickedOutBucket = table[position1];
            table[position1] = newBucket; // Place the new bucket in position1
            newBucket = kickedOutBucket; // The kicked-out bucket now needs to be placed

            // Calculate the alternate position for the kicked-out bucket
            position1 = (position1 == hash1(newBucket.getBucKey())) ? hash2(newBucket.getBucKey()) : hash1(newBucket.getBucKey());
            attempts++;
        }

        // Step 3: Rehash if a cycle is detected
        rehash();
        put(newBucket.getBucKey(), newBucket.getValue()); // Retry inserting the kicked-out bucket after rehashing
    }
